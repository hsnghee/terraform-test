...
**주의 1**: Ubuntu 22.04 기본 저장소엔 `docker compose`(v2, 띄어쓰기) 플러그인이 없어서, `apt install docker-compose`로 설치되는 건 v1(하이픈 명령어 `docker-compose`)입니다. 아래는 전부 하이픈 버전 기준입니다.

**주의 2**: `docker-compose` v1.29.2는 최신 Docker 엔진과 궁합이 안 맞아 Profile을 바꿔서 `up -d`만 다시 실행하면 `KeyError: 'ContainerConfig'` 에러가 날 수 있습니다(2026-08-14 실습 중 확인). Profile을 바꿀 때는 **반드시 `down` 먼저 실행해서 완전히 지운 뒤 `up`** 하세요 — 기존 컨테이너를 "제자리 교체"하는 코드 경로를 피할 수 있습니다.
